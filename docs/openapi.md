...
В админке: сайдбар **Swagger** → `/api/docs` (внешняя ссылка). In-app Documentation — `/docs`.
...
Opt-in GraphQL (не часть OpenAPI-спека) — [`graphql.md`](./graphql.md): System → GraphQL, playground `/api/graphql`. Тот же `apiEnabled` / `public.*` / QueryEngine, что REST.
